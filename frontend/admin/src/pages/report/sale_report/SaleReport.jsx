...
const SalesReports = () => {
  ...
  const columns = useMemo(() => {
    ...
    if (hasDateGroup) {
      baseColumns.push({
        ...
        body: (rowData) => (
          <Text>
            {rowData.date ? dayjs(rowData.date).format("YYYY-MM-DD") : "N/A"}
          </Text>
        ),
      });
    }
    ...
    if (hasCustomerGroupGroup) {
      baseColumns.push({
        ...
        style: { minWidth: "150px" },
        body: (rowData) => (
          <Text>{rowData.customer_group_name || "Walk-In Customer"}</Text>
        ),
      });
    }
    ...
    baseColumns.push(
      {
        ...
        body: (rowData) => (
          <Text>{Number(rowData.quantity || 0).toLocaleString()}</Text>
        ),
      },
      {
        field: "subtotal",
        ...
        body: (rowData) => (
          <Text>${Number(rowData.subtotal || 0).toFixed(2)}</Text>
        ),
      },
      {
        field: "inv_discount",
        ...
        body: (rowData) => (
          <Text>${Number(rowData.inv_discount || 0).toFixed(2)}</Text>
        ),
      },
      {
        field: "item_discount",
        ...
        style: { minWidth: "100px", textAlign: "right" },
        body: (rowData) => (
          <Text>${Number(rowData.item_discount || 0).toFixed(2)}</Text>
        ),
      },
      {
        field: "total_sale",
        ...
        style: { minWidth: "120px", textAlign: "right" },
        body: (rowData) => (
          <Text>${Number(rowData.total_sale || 0).toFixed(2)}</Text>
        ),
      },
      {
        field: "total_cost",
        ...
        style: { minWidth: "120px", textAlign: "right" },
        body: (rowData) => (
          <Text>${Number(rowData.total_cost || 0).toFixed(2)}</Text>
        ),
      },
      {
        field: "profit",
        ...
        body: (rowData) => (
          <Text
            style={{
              color: (rowData.profit || 0) >= 0 ? "#52c41a" : "#f5222d",
            }}
          >
  ...
  const handleExportExcel = useCallback(async () => {
    ...
    try {
      ...
      worksheet.addRow([
        `Grouped By: ${appliedFilters.groupBy.join(", ") || "None"}`,
      ]);
      ...
      sales.forEach((sale) => {
        ...
        if (hasDateGroup)
          rowData.push(
            sale.date ? dayjs(sale.date).format("YYYY-MM-DD") : "N/A"
          );
        ...
        if (hasCustomerGroupGroup)
          rowData.push(sale.customer_group_name || "Walk-In Customer");
      ...
      const avgUnitPrice = totals.totalQuantity
        ? totals.totalSubtotal / totals.totalQuantity
        : 0;
      const avgUnitCost = totals.totalQuantity
  ...
  return (
    <Spin
      ...
      tip={exportLoading ? "Exporting..." : "Loading sales data..."}
      size="large"
    >
      <div>
        {/* Header Section */}
          ...
          <Row gutter={[16, 16]} align="middle">
            ...
            <Col xs={24} sm={12} md={6}>
              <Select
                ...
                onChange={(value) =>
                  setPendingFilters((prev) => ({ ...prev, saleType: value }))
                }
                allowClear
                size="large"
              >
            ...
            <Col xs={24} sm={12} md={6}>
              <Select
                ...
                onChange={(value) =>
                  setPendingFilters((prev) => ({ ...prev, groupBy: value }))
                }
                allowClear
            ...
            <Col xs={24} sm={12} md={6}>
              <Select
                ...
                onChange={(value) =>
                  setPendingFilters((prev) => ({ ...prev, salesPerson: value }))
                }
                allowClear
                size="large"
              >
