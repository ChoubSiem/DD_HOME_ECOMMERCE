...
const AddSaleReturn = () => {
      ...
      if (result.success) {
        ...
        if (savedItems) {
          ...
          const items = result.sale.items.map((item) => ({
            ...
            discount_amount:
              Number(item.discount) + Number(item.inv_discount) || 0,
            total:
              Number(item.price) * 1 -
              (Number(item.discount) + Number(item.inv_discount) || 0),
          }));
      ...
      return prevItems.map((item, i) => {
        if (i === index) {
          const newQty = Math.max(
            0,
            Math.min(Number(value), item.original_qty)
          );
          const discountPerItem =
            (item.discount_amount || 0) / item.original_qty;
          return {
            ...item,
            return_qty: newQty,
  ...
  const calculateSubtotal = () => {
    return saleItems.reduce(
      (sum, item) => sum + item.return_qty * item.price,
      0
    );
  };
  ...
  const calculateTotalDiscount = () => {
    return saleItems.reduce((sum, item) => {
      const discountPerItem =
        Number(item.inv_discount || 0) / item.original_qty;
      return (
        sum +
        discountPerItem * item.return_qty +
        item.discount * item.return_qty
      );
    }, 0);
  };
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: "20px" }}>
            ...
            <div style={{ flex: "1 1 45%", minWidth: "250px" }}>
              ...
              <InputText
                ...
                onChange={(e) =>
                  setFormData({ ...formData, saleperson: e.target.value })
                }
                style={{ width: "100%", padding: "8px 12px" }}
              />
            </div>
              ...
              <Column
                ...
                body={(rowData, { rowIndex }) => (
                  <InputNumber
                    ...
                    onValueChange={(e) =>
                      handleQuantityChange(rowIndex, e.value)
                    }
                    min={0}
              ...
              <Column
                ...
                body={(rowData) => (
                  <span>
                    $
                    {(
                      (Number(rowData.discount_amount) /
                        Number(rowData.original_qty)) *
                      rowData.return_qty
                    ).toFixed(2)}
                  </span>
                )}
              ...
              <Column
                ...
                body={(rowData) => (
                  <span style={{ fontWeight: "600" }}>
                    $
                    {(
                      rowData.return_qty * rowData.price -
                      (Number(rowData.discount_amount) /
                        Number(rowData.original_qty)) *
                        rowData.return_qty
                    ).toFixed(2)}
