...
import StockTransferTable from "../../../components/stock_transfer/StockTransferTable";
...
const StockTransferPage = () => {
  ...
  const {handleGetUomTransfer,handleGetUomTransferDelete} = useStock();
  ...
  const hasUOMTransferPermission = permissions.some(
    (p) => p.name === "UomTranser.add"
  );
  ...
  const handleUomTransferData = async()=>{
    let result = await handleGetUomTransfer(token,user.warehouse_id);    
    if (result.success) {
      setTransfers(result.uoms);
    }
  }
    ...
    <div className="product-management">
      ...
      <Card className="header-card">
        <div className="header-content">
          <div>
            <h1 style={{ color: '#52c41a' }}>UOM Transfer</h1>
            <p>Manage your UOM transfer</p>
          </div>
          {hasUOMTransferPermission &&
          <Button 
            onClick={handleAdd} 
            type="primary" 
            icon={<PlusOutlined />} 
