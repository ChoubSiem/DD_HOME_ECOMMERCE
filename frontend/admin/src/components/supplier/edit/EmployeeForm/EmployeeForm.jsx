...
const EmployeeForm = ({ roles, employees, employee, onRefresh }) => {
    const [showPassword, setShowPassword] = useState(false);
    const { handleEmployeeUpdate } = useUser();
    ...
    useEffect(() => {
        ...
        if (employee) {
            setFormData({
                ...
                password: '', // Password intentionally left blank for security
                warehouse_id: employee.warehouse_id || '',
                role_id: employee.role_id || '',
                profileImage: employee.profile_image_url || null,
                parentIds: employee.parents?.map(p => p.id) || [],
                childrenIds: employee.children?.map(c => c.id) || []
            });
            setPreviewImage(employee.profile_image_url || null);
        }
    }, [token, employee]);
    ...
    const validateForm = () => {
        ...
        if (!formData.warehouse_id) newErrors.warehouse = 'Warehouse is required';
        return newErrors;
    };
    ...
    const handleSubmit = async (e) => {
        ...
        setIsLoading(true);
        try {
            const result = await handleEmployeeUpdate(employee.id, formData, token);
            if (result.success) {
                message.success('Employee updated successfully');
                if (onRefresh) onRefresh(); // Refresh parent component if callback provided
                navigate("/employee");
            } else {
                message.error(result.message || 'Failed to update employee');
            }
        } catch (error) {
        ...
        <div className="container-form">
            <div className="form-card card-form">
                <div className="header">
                    <h2 style={{ color: "#52c41a" }}>Update Employee</h2>
                    <p className="header-subtitle">Update employee details</p>
                </div>
                                ...
                                <div className="form-row">
                                    ...
                                    <div className="form-group">
                                        ...
                                        <Select
                                            ...
                                            onChange={(value) => handleChange({ target: { name: 'role_id', value } })}
                                            options={[
